...
(function() {
	...
	function probe(url, method, callback) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				callback(xhr.status === 200 || xhr.status === 0, xhr.responseText);
			}
		};
		xhr.send();
	}
	...
	function probeModuleUrl(name, url, callback) {
		...
		var resolvedPath = requirejs.lookup(name) || requirejs.lookup(base);
		if (resolvedPath) {
			return callback(resolvedPath);
		}

		var queue = [url, base + '/bower.json', base + '/package.json'];
		var next = function() {
			if (!queue.length) {
	...
	requirejs.load = function(context, moduleName, url) {
		// console.log('Requested', moduleName, url);
		var config = context.config;
		if (moduleName in config.paths || reDomain.test(url)) {
			// путь к модулю либо абсолютный, либо явно указан в конфиге: 
			// не будем его проверять
			return rjsLoad.call(this, context, moduleName, url);
		}
